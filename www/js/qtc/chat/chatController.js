/**
function($scope, $timeout, $stateParams, ionicMaterialInk, $ionicLoading, $state, $http, $localstorage, $ionicPopup, $loadingService, $cordovaFileTransfer, $ionicPlatform) {
...
.controller('ChatCtrl', ['$scope', '$rootScope', '$state', '$stateParams', '$ionicActionSheet', '$ionicSideMenuDelegate',
  '$ionicPopup', '$ionicScrollDelegate', '$timeout', '$interval', 'ionicMaterialInk', '$ionicLoading', '$http', '$localstorage', '$loadingService', '$socket',
  function($scope, $rootScope, $state, $stateParams, $ionicActionSheet, $ionicSideMenuDelegate, $ionicPopup, $ionicScrollDelegate, $timeout, $interval,
    ionicMaterialInk, $ionicLoading, $http, $localstorage, $loadingService, $socket) {

        $scope.messages = [];
        $scope.roomId = $stateParams.room;
        $scope.room_name = $stateParams.room_name;
        $scope.$emit('atualizaNomeSalaMenuDireito', $scope.room_name);
        $scope.user_qtc = $localstorage.getObject('usuario');

        // mock acquiring data via $stateParams
        $scope.toUser = {
          _id: $scope.user_qtc._id,
          pic: 'img/venkman.jpg',
          username: 'Venkman'
        }
        ...
        $scope.input = {
            message: localStorage['userMessage-' + $scope.roomId] || ''
        };
        ...
        $scope.connectChatServer = function(){
            $socket.addListener('new_user',function(msg){
                $scope.$emit('adicionaMenuUsuarios', msg);
            });

            $socket.addListener('user_quit',function(msg){
                $scope.$emit('removeMenuUsuarios', msg);
            });

            $socket.addListener('room_users',function(msg){
                $scope.$emit('criarMenuUsuarios', msg);
            });

            $socket.addListener('chat_message',function(msg){
                $scope.messages.push(msg);
                viewScroll.scrollBottom(true);
            });

            $socket.emit('join', {'sala' : {'_id': $scope.roomId}, 'usuario': $scope.user_qtc});
            // $socket.emit('chat_message', {'sala' : {'_id': $scope.roomId}, 'usuario' : $scope.user_qtc, 'texto' : 'texto teste de averiguação'});
        }

        $scope.$on('$ionicView.enter', function() {
            $scope.connectChatServer();
            $timeout(function() {
        ...
        $scope.$on('$ionicView.leave', function() {
            $socket.emit('leave', {'sala': {'_id' : $scope.roomId}, 'usuario': $scope.user_qtc});
            $scope.$emit('criarMenuUsuarios', {'usuarios' : []});
        });

        $scope.$on('$ionicView.beforeLeave', function() {
            if (!$scope.input.message || $scope.input.message === '') {
                localStorage.removeItem('userMessage-' + $scope.roomId);
            }
        });

        $scope.$watch('input.message', function(newValue, oldValue) {
            if (!newValue) newValue = '';{
                localStorage['userMessage-' + $scope.roomId] = newValue;
            }
        });
        ...
        $scope.sendMessage = function() {
            ...
            $socket.emit('chat_message', {'sala' : {'_id': $scope.roomId}, 'usuario' : $scope.user_qtc, 'texto' : $scope.input.message});
            $scope.input.message = '';
